...
const mongoose = require('mongoose'); // Dodato
...
// OVDE STAVI SVOJ LINK KOJI SI KOPIRAO (Zameni <password> i ostalo)
const MONGO_URI = "mongodb+srv://admin:Sifra123@cluster0.abcde.mongodb.net/neon_casino?retryWrites=true&w=majority";

mongoose.connect(MONGO_URI)
    .then(() => console.log("✅ POVEZAN NA MONGODB ATLAS"))
    .catch(err => console.error("❌ GREŠKA PRI POVEZIVANJU NA MONGO:", err));

// --- MODEL KORISNIKA (Zamenjuje users.json) ---
...
// --- HTTP RUTE (Modernizovane za MongoDB) ---
...
app.post('/register', async (req, res) => {
    ...
    try {
        ...
        console.log(`✅ Registrovan u bazu: ${username}`);
        res.json({ status: "uspešno", poruka: "Registracija uspela!" });
    } catch (err) {
        res.status(400).json({ status: "greška", poruka: "Korisnik već postoji ili su podaci neispravni!" });
    }
});
...
io.on('connection', (socket) => {
    ...
    socket.on('join-room', (data) => {
        ...
        if (!gameRooms[room]) {
            gameRooms[room] = {
                ...
                p1: { id: socket.id, name: user, ready: false, score: 0 },
                p2: { id: null, name: "Čekanje...", ready: false, score: 0 },
    ...
    // Ostatak tvoje player-ready, sync-pos i game-over logike ostaje isti kao u prethodnoj poruci...
    // Ovde je bitno da u game-over pozoveš dodeliNagradu koja je sad asinhrona
...
server.listen(PORT, () => console.log(`🚀 NEON SERVER ONLINE NA PORTU ${PORT}`));